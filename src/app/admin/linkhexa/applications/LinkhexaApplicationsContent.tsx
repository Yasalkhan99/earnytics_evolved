...
type App = { id: number; publisher_id: string; programme_id: string; status: string; applied_at: string };

export default function LinkhexaApplicationsContent() {
  ...
  const load = async (s = status) => {
    setLoading(true);
    const res = await fetch(`/api/admin/linkhexa/applications?status=${s}`, { credentials: "include" });
  ...
  const update = async (id: number, newStatus: string) => {
    await fetch("/api/admin/linkhexa/applications", { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status: newStatus }) });
    void load();
  };
  ...
  const approveAll = async () => {
    ...
    await Promise.all(
      pending.map(a =>
        fetch("/api/admin/linkhexa/applications", {
      ...
      <div className="flex gap-2">
        {["pending","approved","rejected"].map(s => (
          <button key={s} onClick={() => { setStatus(s); void load(s); setMsg(null); }}
            className={`rounded-full px-3 py-1 text-xs font-semibold ${status === s ? "bg-teal-600 text-white" : "bg-gray-100 text-gray-600"}`}>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-xs font-black uppercase tracking-wider text-gray-400">
            <tr>{["Publisher","Programme","Status","Applied","Actions"].map(h => <th key={h} className="px-4 py-3 text-left">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            ...
            : apps.map(a => (
              <tr key={a.id} className="hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3 text-gray-900">{a.programme_id}</td>
