...
export default function YieldkitManageContent() {
  ...
  const loadStats = async () => {
    setLoading(true);
    const res = await fetch("/api/admin/yieldkit/stats", { credentials: "include" });
  ...
  const runSync = async (action: string) => {
    ...
    const urlMap: Record<string, string> = {
      campaigns:    "/api/admin/yieldkit/sync-campaigns",
      transactions: "/api/admin/yieldkit/sync-transactions",
      rollup:       "/api/admin/yieldkit/rebuild-rollup",
    };
    ...
    if (!res.ok) {
      ...
      setSyncMsg(`⚠ ${data.warning}`);
    } else {
      setSyncMsg(`✓ Done${data.upserted != null ? ` — ${data.upserted} records synced` : ""}`);
    }
    setSyncing(null);
    await loadStats();
  };
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Sync campaigns and transactions from the Yieldkit API.</p>
      </div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <div className="flex flex-wrap gap-4">
            {Object.entries(stats.commissionByCurrency).map(([c, v]) => (
              <div key={c}>
                <p className="text-xl font-extrabold text-indigo-600">{formatMoney(v, c)}</p>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          { id: "campaigns",    label: "Sync Campaigns",    desc: "Fetch all active campaigns from Yieldkit Advertiser API.", icon: "🌐" },
          { id: "transactions", label: "Sync Transactions", desc: "Fetch recent commissions and attribute to publishers.", icon: "💳" },
          { id: "rollup",       label: "Rebuild Rollup",    desc: "Rebuild the publisher earnings daily summary table.", icon: "📊" },
        ].map((a) => (
      ...
      {syncMsg && (
        <div className={`rounded-xl px-4 py-3 text-sm font-medium ${
          syncMsg.startsWith("✓") ? "bg-emerald-50 text-emerald-700 border border-emerald-200"
          : syncMsg.startsWith("⚠") ? "bg-amber-50 text-amber-700 border border-amber-200"
          : "bg-red-50 text-red-600 border border-red-200"
        }`}>
          {syncMsg}
          {syncMsg.startsWith("⚠") && (
            <a href="/admin/yieldkit/connection" className="ml-2 underline font-semibold">
