...
type Campaign = {
  ...
  commission_type: string | null; commission_rate: string | null; description: string | null;
};
...
export default function YieldkitBrandDetailContent({ advertiserId }: { advertiserId: string }) {
  ...
  const load = useCallback(async (showSpinner = true) => {
    ...
    const res = await fetch(`/api/publisher/yieldkit/brands/${advertiserId}`, { credentials: "include" });
  ...
  const applyNow = async () => {
    ...
    setApplying(true);
    try {
      const res = await fetch("/api/publisher/yieldkit/apply", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ advertiserId: campaign.advertiser_id }),
      });
  ...
  const createLink = async () => {
    ...
    try {
      const res = await fetch("/api/publisher/yieldkit/go-links", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ advertiserId: campaign.advertiser_id, landingPage: landingPage.trim() || undefined }),
      });
      const data = await res.json();
        ...
        <aside className="w-full lg:w-56 lg:shrink-0 space-y-4">
          <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
            ...
            <div className="flex flex-col items-center p-5 text-center">
              ...
              <p className="mt-0.5 text-[11px] text-gray-400">Yieldkit · {campaign.country ?? "Global"}</p>
          ...
          {campaign.description && (
            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
              <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-gray-400">About</p>
              <p className="text-xs leading-relaxed text-gray-600 line-clamp-6">{campaign.description}</p>
            </div>
          )}
        </aside>
              ...
              <div className="divide-y divide-gray-50">
                ...
                {campaign.description && (
                  <div className="px-5 py-4">
                    ...
                    <p className="text-sm leading-relaxed text-gray-700">{campaign.description}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              ...
              <div className="bg-indigo-50 border-t border-indigo-100 px-5 py-3">
                <p className="text-xs text-indigo-700">
                  Commissions are tracked via Yieldkit&apos;s <code>yk_tag</code> sub-ID — your go-link slug is the tag for attribution.
                </p>
              </div>
            </div>
          )}
                ...
                <>
                  <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                    <div className="border-b border-gray-50 px-5 py-4">
                      ...
                      <p className="mt-0.5 text-xs text-gray-400">Your go-link slug is used as the <code>yk_tag</code> for Yieldkit attribution.</p>
                    </div>
                    <div className="p-5 space-y-4">
                      ...
                      <div>
                        ...
                        <input value={landingPage} onChange={(e) => setLandingPage(e.target.value)}
                          placeholder={campaign.url ? `e.g. ${campaign.url}/sale` : "https://example.com/page"}
                        ...
                        <p className="mt-1 text-[11px] text-gray-400">Leave blank to use the default campaign URL. The Yieldkit Redirect API will affiliate the URL.</p>
                      </div>
